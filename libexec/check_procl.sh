...
# check_procl.sh,v 1.2 2013/10/31 15:19
...
do
    case "$1" in
        -p|--process)
            ...
            ;;
        -e|--exclude-process)
            if [ -n "$exclude_process_name" ]; then
...
if [ -z "$wt_value1" -o -z "$wt_value2" -o -z "$wt_value3" ]; then
    echo "Bad expression in the WARNING field : $wt"
    help_usage
    exit 3
fi

if [ "`echo $wt_value1|tr -d \"[:digit:]\"`" != "" -o "`echo $wt_value2|tr -d \"[:digit:]\"`" != "" -o "`echo $wt_value3|tr -d \"[:digit:]\"`" != "" ]; then
    echo "Bad expression in the WARNING field : $wt"
    help_usage
    exit 3
fi

if [ -z "$ct_value1" -o -z "$ct_value2" -o -z "$ct_value3" ]; then
...
if [ "`echo $ct_value1|tr -d \"[:digit:]\"`" != "" -o "`echo $ct_value2|tr -d \"[:digit:]\"`" != "" -o "`echo $ct_value3|tr -d \"[:digit:]\"`" != "" ]; then
    echo "Bad expression in the CRITICAL field : $ct"
    help_usage
    exit 3
fi
...
case "$process_name" in
    %all%)
        if [ -z "$exclude_process_name" ]; then
...
do
    ...
    if [ $tt -eq 3 ]
    then
        if [ $total3 -ge 60 ]; then
            ((total2++))
            ((total3-=60))
        fi
        if [ $total2 -ge 60 ]; then
            ((total1++))
            ((total2-=60))
        fi
    else
        if [ $total2 -ge 10 ]; then
            ((total1++))
            ((total2-=10))
        fi
    fi
...
if [ $crit -eq 1 ]; then
    echo "CRITICAL - total $type_arg_aff for process `echo $process_name|tr \"|\" \",\"` : $return_total |  $type_arg_aff=$return_total;$wt_value1;$ct_value1"
    exit 2
elif [ $warn -eq 1 ]; then
    echo "WARNING - total $type_arg_aff for process `echo $process_name|tr \"|\" \",\"` : $return_total |  $type_arg_aff=$return_total;$wt_value1;$ct_value1"
    exit 1
else
    echo "OK - total $type_arg_aff for process `echo $process_name|tr \"|\" \",\"` : $return_total |  $type_arg_aff=$return_total;$wt_value1;$ct_value1"
    exit 0
fi
