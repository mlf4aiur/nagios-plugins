...
our $cachefile = '/var/tmp/'; # completed later
our %processes = ();
...
sub check_for_matches {
    ...
    foreach my $key ( keys(%processes) ) {
        ...
        # elsif ( defined($opt_regex) && $processes{$key}->{name} =~ /$opt_regex/o ) { $match++; }
            ...
            # filter this program out
            # if ( $processes{$key}->{name} !~ /$PROGNAME/o ) {
            if ( $processes{$key}->{name} !~ /$0/o ) {
                $match++;
            }
        }
...
if ( ($opt_warning->[0] > 0 && $opt_warning->[0]  >  $count)
  || ($opt_warning->[1] > 0 && $opt_warning->[1]  <= $count) ) {
    $message = "WARNING - $count process(es) found resembling '". ($opt_command || $opt_regex);
    $exit = $ERRORS{WARNING};
}
if ( ($opt_critical->[0] > 0 && $opt_critical->[0]  >  $count)
  || ($opt_critical->[1] > 0 && $opt_critical->[1]  <= $count) ) {
    $message = "CRITICAL - $count process(es) found resembling '". ($opt_command || $opt_regex);
